...
internal object FormatUtils {
    ...
    fun fastFormat(d: Double, precision: Int): String {
        val posPrecision = abs(precision.toDouble()).toInt()
        val roundUpVal: Double = abs(d) * 10.0.pow(posPrecision.toDouble()) + 0.5
    ...
    private fun bigDecFormat(d: Double, precision: Int): String {
        var formatStr: String =
            BigDecimal(d.toString()).setScale(abs(precision), RoundingMode.HALF_UP)
                .toString()
        if (precision == 0) {
            return formatStr
        }
